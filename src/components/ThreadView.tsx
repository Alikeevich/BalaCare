...
const ThreadView: React.FC<ThreadViewProps> = ({ post, onClose, onPostUpdate, onCommentAdded, onStartChat }) => {
    ...
    <div className={`relative ${depth > 0 ? 'ml-8 mt-4' : 'mt-6'}`}>
       ...
       <div className="flex gap-3">
          <div className="flex-shrink-0">
             <div 
                onClick={() => comment.user_id && comment.user_id !== user?.id && onStartChat(comment.user_id)}
          ...
          <div className="flex-1">
             <div className="bg-gray-50 p-3 rounded-2xl rounded-tl-sm inline-block min-w-[200px] max-w-full">
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] bg-white flex flex-col h-[100dvh]">
      ...
      {/* Header */}
      <div className="flex-none px-4 py-3 border-b border-gray-100 flex items-center gap-4 bg-white/90 backdrop-blur-md pt-safe-top z-10">
        <button onClick={onClose} className="p-2 -ml-2 hover:bg-gray-100 rounded-full active:scale-95 transition-transform">
      ...
      <div className="flex-1 overflow-y-auto bg-white" style={{ minHeight: 0 }}>
        ...
        <div className="h-2 bg-gray-50 w-full mb-4 border-t border-b border-gray-100"></div>
      ...
      <div className="flex-none bg-white border-t border-gray-200 p-3 z-20 w-full" style={{ paddingBottom: 'calc(20px + env(safe-area-inset-bottom))' }}>
        {replyTo && (
           <div className="flex justify-between items-center bg-purple-50 px-4 py-2 rounded-t-xl text-xs text-purple-700 mb-1 mx-1 border border-purple-100 border-b-0">
              <span className="flex items-center gap-1 font-medium"><CornerDownRight className="w-3 h-3"/> Ответ для <b>{replyTo.name}</b></span>
              <button onClick={() => setReplyTo(null)} className="p-1 hover:bg-purple-100 rounded-full"><X className="w-3 h-3"/></button>
           </div>
        )}
        <div className="flex items-end gap-2 bg-gray-50 p-2 rounded-3xl border border-gray-200 focus-within:border-purple-400 transition-all shadow-sm">
           <div className="w-9 h-9 rounded-full bg-gray-200 overflow-hidden flex-shrink-0 mb-0.5 ml-0.5">
           ...
           <button 
             ...
             className={`w-10 h-10 rounded-full flex items-center justify-center text-white disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed transition-all shadow-md mb-0.5 ${newComment.trim() ? 'bg-purple-600 hover:bg-purple-700 active:scale-95' : 'bg-gray-300'}`}
           >
