...
const HomePage: React.FC = () => {
  // Данные меню
  const menuItems = [
    { title: "Обучение", sub: "Видео и статьи", icon: BookOpen, color: "bg-blue-50 text-blue-600" },
    { title: "Специалисты", sub: "Онлайн запись", icon: Heart, color: "bg-pink-50 text-pink-600" },
    { title: "Сообщество", sub: "Общение мам", icon: Users, color: "bg-purple-50 text-purple-600" },
    { title: "Магазин", sub: "Товары детям", icon: ShoppingBag, color: "bg-orange-50 text-orange-600" },
  ];
    ...
    <div className="pb-8 space-y-6">
      ...
      <div className="px-6 grid grid-cols-2 gap-4 mt-8">
        {menuItems.map((item, i) => {
          // ВАЖНО: Присваиваем иконку переменной с Большой Буквы
          const IconComponent = item.icon;
          
          return (
            <button key={i} className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-start hover:shadow-md transition-all active:scale-95">
              <div className={`${item.color} p-3 rounded-2xl mb-3`}>
                {/* Теперь рендерим компонент корректно */}
