...
const LearningPage: React.FC = () => {
  ...
  const fetchContent = async () => {
    try {
      // Запрашиваем контент и название типа
  ...
  const articles = content.filter(c => c.content_types?.name === 'article' || !c.content_types); // fallback

  const tags = ["#Аутизм", "#Речь", "#Моторика", "#Питание", "#Сон"];
  ...
  return (
    <div className="pt-6 pb-24 space-y-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex gap-2 px-6 overflow-x-auto pb-2 no-scrollbar">
        {tags.map((tag, i) => (
          <span key={i} className="whitespace-nowrap px-4 py-2 bg-white border border-gray-100 rounded-full text-sm font-bold text-gray-600 shadow-sm hover:text-purple-600 hover:border-purple-200 transition-colors cursor-pointer active:scale-95">
            {tag}
          </span>
        ))}
      </div>
        ...
        <>
          ...
          {videos.length > 0 && (
            <div>
              <div className="flex justify-between items-end px-6 mb-4">
                ...
                <span className="text-xs font-bold text-purple-600 cursor-pointer">Все видео</span>
              </div>
              ...
              <div className="flex gap-4 overflow-x-auto px-6 pb-6 snap-x">
                {videos.map((v) => (
                  <div key={v.id} className="snap-center shrink-0 w-72 group cursor-pointer">
                    ...
                    <h3 className="font-bold text-gray-800 leading-snug group-hover:text-purple-600 transition-colors line-clamp-2">{v.title}</h3>
                  </div>
                ))}
              </div>
            </div>
          ...
          {articles.length > 0 && (
            <div className="px-6 space-y-4">
              <h2 className="text-xl font-bold flex items-center gap-2">
                <FileText className="w-5 h-5 text-blue-500" /> Статьи и чеклисты
              </h2>
              {articles.map((a) => (
                <div key={a.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex gap-4 hover:shadow-md transition-shadow cursor-pointer active:scale-[0.98]">
                  <div className="w-20 h-20 bg-gray-100 rounded-xl shrink-0 overflow-hidden">
                    {a.thumbnail_url && <img src={a.thumbnail_url} className="w-full h-full object-cover" alt="" />}
                  </div>
